...
You'll need to install [`hls.js`][hlsjs] if you're using the HLS provider
...
The HLS provider will default to loading `hls.js` from [JSDelivr][jsdelivr]. We load the default
...
events on the [player][player] while loading hls.js:
