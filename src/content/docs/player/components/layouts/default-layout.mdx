...
See the [installation guide][install] to setup the Default Layout.

[install]: /docs/player/getting-started/installation?styling=default-layout&provider=video

## Demo

You can try out the Default Layout on our [player demo](/player/demo?layout=default&framework=wc) page.
...
<StyleImports type="default-layout" client:load />

<JSPlayerCode type="default-layout" client:load />
...
theme by setting the `color-scheme` attribute on the layout like so:
