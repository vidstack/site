...
This is where [container queries](#container-queries) and [player queries](#player-queries) are
preferable.
...
Player Queries are a feature we ship to enable matching against the current media state using
strings which support CSS media-query-like syntax. They are generally used when working with
layouts.

Player Queries support the following syntax:

- Comparators: `(width < 576) or (height <= 380)`
- And: `(view-type: video) and (stream-type: on-demand)`
- Or: `(fullscreen: true) or (orientation: landscape)`
- Media Queries: `(fullscreen: true) and @media (min-width: 992px)`

Do note, all **[media player state][player-state]** properties are available when writing queries.

:::danger
Player Queries use `eval` under the hood. **DO NOT** inject unsanitized content into query strings.
:::
